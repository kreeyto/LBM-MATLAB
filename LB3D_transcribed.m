...
ii = 2:nx-1; jj = 2:ny-1; kk = 2:nz-1; % aux idx
isfluid(ii,jj,kk) = 1;
...
for i = 2:nx-1
    for j = 2:ny-1
        for k = 2:nz-1
            Ri = sqrt((i-nx/2)^2/farg + (j-ny/2)^2 + (k-nz/2)^2);
...
for t = 1:nsteps
    ...
    for i = 2:nx-1
        for j = 2:ny-1
            for k = 2:nz-1
                ...
                ffx(i,j,k) = sigma .* curvature(i,j,k) .* normx(i,j,k) .* indicator(i,j,k) * bool_ind;
                ffy(i,j,k) = sigma .* curvature(i,j,k) .* normy(i,j,k) .* indicator(i,j,k) * bool_ind;
                ffz(i,j,k) = sigma .* curvature(i,j,k) .* normz(i,j,k) .* indicator(i,j,k) * bool_ind;
            end
        end
    end
    ...
    for i = 2:nx-1
        for j = 2:ny-1
            for k = 2:nz-1
                ux(i,j,k) = ( ...
                    f(i,j,k,2) - f(i,j,k,3) + f(i,j,k,8) - f(i,j,k,9) + f(i,j,k,10) - ...
                    f(i,j,k,11) + f(i,j,k,14) - f(i,j,k,15) + f(i,j,k,16) - f(i,j,k,17) ...
                ) ./ rho(i,j,k) + ffx(i,j,k) * 0.5 ./ rho(i,j,k);
                uy(i,j,k) = ( ...
                    f(i,j,k,4) - f(i,j,k,5) + f(i,j,k,8) - f(i,j,k,9) + f(i,j,k,12) - ...
                    f(i,j,k,13) - f(i,j,k,14) + f(i,j,k,15) + f(i,j,k,18) - f(i,j,k,19) ...
                ) ./ rho(i,j,k) + ffy(i,j,k) * 0.5 ./ rho(i,j,k);
                uz(i,j,k) = ( ...
                    f(i,j,k,6) - f(i,j,k,7) + f(i,j,k,10) - f(i,j,k,11) + f(i,j,k,12) - ...
                    f(i,j,k,13) - f(i,j,k,16) + f(i,j,k,17) - f(i,j,k,18) + f(i,j,k,19) ...
