...
ny = 50; % Número de pontos na direção y.
nz = 50; % Número de pontos na direção z.
...
g = zeros(nx, ny, nz, 7); % Campos de distribuição de fase (7 direções).
...
p = [1/3, 1/18, 1/18, 1/18, 1/18, 1/18, 1/18, ...
    1/36, 1/36, 1/36, 1/36, 1/36, 1/36, 1/36, ...
    1/36, 1/36, 1/36, 1/36, 1/36]; % Pesos para cada direção.

p_g = [1/3, 1/6, 1/6, 1/6, 1/6, 1/6, 1/6]; % Pesos para as direções de fase
opp = [0, 2, 1, 4, 3, 6, 5, 10, 9, 12, 11, 14, 13, 16, 15, 18, 17]; % Direções opostas. 
...
g(:,:,:,1) = p_g(1) * fi(:,:,:); % Direção 0
g(:,:,:,2) = p_g(2) * fi(:,:,:); % Direção [1, 0, 0]
g(:,:,:,3) = p_g(3) * fi(:,:,:); % Direção [-1, 0, 0]
g(:,:,:,4) = p_g(4) * fi(:,:,:); % Direção [0, 1, 0]
g(:,:,:,5) = p_g(5) * fi(:,:,:); % Direção [0, -1, 0]
g(:,:,:,6) = p_g(6) * fi(:,:,:); % Direção [0, 0, 1]
g(:,:,:,7) = p_g(7) * fi(:,:,:); % Direção [0, 0, -1]
