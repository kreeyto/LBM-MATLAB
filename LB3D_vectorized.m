...
%% Visualização
if slicebool == 3
    hVol = volshow(fi, 'RenderingStyle', 'VolumeRendering');
    viewer = hVol.Parent;
    hFig = viewer.Parent;
end
%% Loop de Simulação
...
for t = 1:nsteps
    ...
    normx = grad_fix ./ (mod_grad + 1e-9);
    normy = grad_fiy ./ (mod_grad + 1e-9);
    normz = grad_fiz ./ (mod_grad + 1e-9);
    indicator = mod_grad;   
    ...
    % Momentos    
    u = sum(f(:,:,:,[2,16,10,8,14]),4) - sum(f(:,:,:,[3,11,17,15,8]),4);
